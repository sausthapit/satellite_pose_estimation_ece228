...
if __name__ == "__main__":
    # models_name = ['nvidia', 'baseline', 'vgg16','satelllitenet']
    models_name = ['satellitenet']
    image_size = (224,224)
    batch_size = 32
    epochs = 15
    train = 1
    ...
    steps_per_epoch = int(len(full_dataset) / batch_size)

    train_generator=DataLoader(full_dataset,batch_size=batch_size,shuffle=True)
    T = [5, 10, 15, 20, 25]
    #T = [25]
    for model_name in models_name:
        for t in T:
            distillation_net = myModel(type_='student')
            distillation_net = distillation_net.to(device)
            original_net = myModel(type_='student')
            original_net.load_state_dict(torch.load(model_name + '.pth',map_location=device))
                ...
                for epoch in range(epochs):
                    ...
                    for step, sample_batched in enumerate(train_generator):
                        if step <= steps_per_epoch:
                            ...
                            _,_, teacher_dist = original_net(batch_x)
