...
def exp(net, model_name, attack, test_dataset, device):
    ...
    if model_name == 'baseline':
        ...
        original_net = Vgg16()
    elif model_name == 'satellitenet':
        original_net= myModel()
    original_net.load_state_dict(torch.load(model_name + '.pth'))
...
if __name__ == "__main__":
    ...
    models_name = ['baseline','nvidia','vgg16','satellitenet']
            ...
            for attack in attacks:
                ...
                if train != 0:
                    if train == 2:
                        ...
                        train_data_loader = torch.utils.data.DataLoader(test_set, batch_size=1, shuffle=True)
                        criterion = nn.L1Loss()
                        ...
                        for epoch in range(epochs):
                            total_loss = 0
                            for step, sample_batched in enumerate(train_data_loader):
