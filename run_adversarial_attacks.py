...
from adversarial.fgsm_attack import fgsm_attack_fun,fgsm_attack
np.random.seed(0)
try:
...
'''
class myorigModel(models.ResNet):
    def __init__(self, block, layers, num_classes=7):

        super(myorigModel, self).__init__(block, layers, num_classes)
    ...
    #def load_weights():
        ...
        #model = models.ResNet(Bottleneck, [3, 4, 6, 3], 7)
        #if pretrained:
...
def fgsm_attack_test(model, image, target, device, epsilon=0.01, image_size=(128, 128)):
    ...
    plt = generate_image(image.detach().cpu().numpy().transpose(0, 2, 3, 1)[0, :, :, :],
                         perturbed_image.detach().cpu().numpy().transpose(0, 2, 3, 1)[
                         0, :, :, :], noise.detach().cpu().numpy().transpose(0, 2, 3, 1)[0, :, :, :],
                         steer.detach().cpu().numpy()[0][0], adv_output.detach().cpu().numpy()[0][0], image_size)
...
def fgsm_ex(test_dataset, model, model_name, target, device, num_sample, image_size=(128, 128)):
    ...
    for i, sample in enumerate(test_dataset):
        diff, plt_, norm_noise, _ = fgsm_attack_test(model, sample['image'], target, device,
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, labels in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):  # gradient enabled only for train mode
                    ...
                    poseLoss, posLoss, orieLoss = PoseNetLoss(outputs[1], outputs[0], labels.float()[:, 4:],
                                                              labels.float()[:, :4], \
                                                              sx=0.0, sq=-1.5)
...
def main(speed_root, epochs, batch_size, train):
    ...
    test_data_loader = DataLoader(test_set, batch_size=1, shuffle=False, num_workers=8)

    # Perform attack
    for x, y in test_data_loader:
        # x, y = x.to(device), y.to(device)
        diff, perturbed_image, location, adv_output, noise = fgsm_attack(trained_model, x, 0.3, device, epsilon=0.01,
                                                                         image_size=(224, 224))
        print("real:", location)
        print("adv:", adv_output)
        print("diff", diff)

    fgsm_ast, diff = fgsm_ex(test_data_loader, trained_model, "baseline", target, device, num_sample, image_size)
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--trained_path', help='Path to the trained model. If not given will train from scratch', default=None)
    parser.add_argument('--train', help='Train from Scratch', default=False)
    args = parser.parse_args()

    main(args.dataset, int(args.epochs), int(args.batch), bool(args.train))
